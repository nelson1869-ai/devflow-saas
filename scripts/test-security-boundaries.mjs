...
// 1. Pure Security Core (No DB dependency)
import {
  USER_ROLES,
  isUserRole,
  validateUserRole,
...
// 2. Setup Isolated Temporary Database Path BEFORE importing application modules
...
try {
  ...
  assert(
    Array.isArray(USER_ROLES) && USER_ROLES.length === 3,
    "USER_ROLES contains exactly 3 allowed roles",
  );
  ...
  assert(
    !viewerAuth.authorized,
    "Viewer persona is rejected from admin actions",
  );
  ...
  console.log("\nTEST 3: API Key Scope Strict Allowlist");
  assert(
    Array.isArray(SUPPORTED_API_SCOPES) && SUPPORTED_API_SCOPES.length === 3,
    "SUPPORTED_API_SCOPES contains exactly 3 allowed scopes",
  );
  ...
  // Dynamically import database and real security access functions with isolated DB
  const dbModule = await import("../src/app/devflow-saas/lib/db.ts");
  db = dbModule.db;
  ...
  assert(
    validProjectAccess.authorized &&
      validProjectAccess.project?.id === "proj-1",
  ...
  const validMsAccess = checkDemoMilestoneAccess(
    msTestId,
    msTestProj1,
    "org-1",
  );
  ...
  // --------------------------------------------------------------------------
  // GUARANTEED TEARDOWN & CLEANUP OF ISOLATED TEST DATABASE
  // --------------------------------------------------------------------------
  if (db) {
    try {
      db.close();
    } catch (err) {
      console.warn("Notice: Closing database connection:", err);
    }
  }
  ...
  try {
    ...
    console.warn("Notice: Cleanup of temporary test database files:", err);
  }
}
