...
export function SprintBurndownTracker({
  ...
  if (!activeMilestone) {
    return (
      <section className="rounded-2xl border border-slate-800/80 bg-slate-900/40 p-8 text-center space-y-3">
  ...
  const maxTaskY = Math.max(activeMilestone.totalTasks, 6);
  const series = activeMilestone.burndownSeries;
  ...
  // Area under actual curve for gradient fill
  const firstPoint = actualPoints[0];
  const lastPoint = actualPoints[actualPoints.length - 1];
  const areaPath = `M ${firstPoint.x},${firstPoint.y} L ${actualPointsStr.replace(/ /g, " L ")} L ${lastPoint.x},${paddingY + plotHeight} L ${firstPoint.x},${paddingY + plotHeight} Z`;
    ...
    <section aria-labelledby="burndown-heading" className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-6 space-y-6 shadow-sm">
        ...
        <div className="grid gap-3 sm:grid-cols-4">
          ...
          <div className="rounded-xl border border-slate-800/80 bg-slate-950/60 p-3">
            <span className="text-[11px] text-slate-400">Sprint Progress</span>
